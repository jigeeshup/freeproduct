...
class C4B_Freeproduct_Model_Observer
{
    ...
    public function salesruleValidatorProcess(Varien_Event_Observer $observer)
    {
        ...
        try {
            ...
            $followQty = $this->getrulefollowcount($rule->getId());
            $qty = $qty * $followQty;
    ...
    public static function getrulefollowcount($ruleid)
    {
        ...
        foreach ($cart->getAllItems() as $item) {
            $itemOb = Mage::getModel('sales/quote_item')->load($item->getId());
            ...
            { 
                $result = array_intersect($category_id, $value);
            if(sizeof($result)>0)
                  {
                    $i += $itemOb->getQty();
