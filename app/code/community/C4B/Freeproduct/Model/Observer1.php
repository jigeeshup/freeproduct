...
class C4B_Freeproduct_Model_Observer
{
	 ...
	 * @return $count
     */
    public function getrulefollowcount($rule_id)
    {
        $cart = Mage::getModel('checkout/cart')->getQuote();
        $rule = Mage::getModel('salesrule/rule')->load($rule_id); 
		$count=0;		
        //print_r(sizeof($cart->getAllVisibleItems());die(); 
        foreach ($cart->getAllItems() as $item) {
            echo $item->getId() .$item->getProduct()->getName(); echo '<br>';
            echo '<br>';
           // echo $productPrice = $item->getProduct()->getPrice();
           echo $rule->getActions()->validate($item);
...
foreach ($cart->getAllVisibleItems() as $item1) 
{ 
    //print_r($item1->getId());
   // die('you are here');
   echo '<br>'. $item1->getName();
$rule = Mage::getModel('salesrule/rule')->load($rule_id); 
$item = Mage::getModel('sales/quote_item')->load($item1->getId());
//echo $item->getCount();
            if ($rule->getActions()->validate($item)) {
                $count++;
            }               	//echo '<br>'.$rule->getName().':'.$count;
}	
...
return 	$count;
    ...
    public function salesruleValidatorProcess(Varien_Event_Observer $observer)
    {
      ...
      if ($rule->getSimpleAction() != C4B_Freeproduct_Model_Consts::ADD_GIFT_ACTION|| $item->getIsFreeProduct()|| $rule->getIsApplied())
        { 
            return;
        }
        ...
        try {
            ...
            foreach ($skus as $sku) {
				// $itqty=$this->getrulefollowcount($rule->getId());	
               // $qty =floor($itqty/$qty); 				 
